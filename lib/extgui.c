...
static int barItem(int id, double x, double y, double w, double h, char *label, int selected)
{
    ...
    if (inBox(gs_UIState.mousex, gs_UIState.mousey, x, x + w, y, y + h)) {
        ...
        if (!gs_UIState.actingMenu && (gs_UIState.clickedItem == id || gs_UIState.clickedItem == 0)
                         ...
                         double progress)
{
    ...
    // Handle drag event
    if (inBox(gs_UIState.mousex, gs_UIState.mousey, gutterX + gutterW / 10, gutterX + gutterW * 9 / 10,
