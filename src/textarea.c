...
void addCodeToEditor(Editor *editor, FILE *fp, char *filePath)
{
    ...
    editor->filePath[editor->fileCount] = (char *)malloc(sizeof(char) * (strlen(
            filePath) + 1));
    ...
    form->viewProgress = 0;
    form->selectLeftPos.r = form->selectLeftPos.c = form->selectRightPos.r =
                                form->selectRightPos.c = 0;
...
void drawEditor(Editor *editor)
{
    ...
    for (idx = 1; idx <= editor->fileCount; idx++) {
        ...
        double scale = form->h / textFontHeight / (form->passage->passList.listLen ?
                       form->passage->passList.listLen : 1);
        if (scale < 1) {
            form->viewProgress = vertivalScrollBar(GenUIID(idx), form->x, form->y, form->w, form->h,
                                                   scale,
                                                   form->viewProgress);
    ...
    MovePen(winWidth - TextStringWidth(tmpStr),
            winHeight - editor->menuHeight + GetFontAscent() / 2);
...
static void drawEditorComplete(Editor *editor)
{
    ...
    if (offset == 0 && form->realCaretPos.c >= 1) {
        ...
        listy = winHeight - editor->barHeight - (-form->startLine + form->realCaretPos.r + 2) *
                textFontHeight;
        listw = TextStringWidth("a") * 20;
        listh = textFontHeight;
        int selection = completeList(GenUIID(0), listx, listy, listw, listh, labels,
                                     list->listLen);
        ...
        if (selection != -1) {
            ...
            addString(form->passage, labels[selection] + token->length, form->realCaretPos.r,
                      form->realCaretPos.c + 1);
...
void drawEditorSelection(EditorForm *form)
{
    ...
    while (nowRol <= rRC.r) {
        double x = form->x + indexLength,
               y = form->h - textFontHeight * (nowRol - form->startLine + 1),
               w = form->w, h = textFontHeight;
        //Draw background
...
static PosRC matchCharForward(Passage *passage, int rows, int cols, char thisChar,
                              char targetChar)
{
...
static PosRC matchCharBackward(Passage *passage, int rows, int cols, char thisChar,
                               char targetChar)
{
...
static void drawSymbolMatch(EditorForm *form)
{
    ...
    int offset = 0;
    double x1 = form->x + indexLength + (form->realCaretPos.c - 1) * TextStringWidth("a"),
           y1 = form->h - (form->realCaretPos.r - form->startLine + 1) * textFontHeight;
...
static void drawEditorMenu(Editor *editor)
{
    ...
    selection = menuList(GenUIID(0), x, y, w, wlist, h, menuListEdit,
    ...
    case 2:
        ...
        break;
    case 3:
        if (curForm->inSelectionMode) Copy(curForm);
        break;
    case 4:
        if (curForm->inSelectionMode) Cut(curForm);
        break;
    case 5:
        Paste(curForm);
        break;
    ...
    selection = menuList(GenUIID(0), x, y, w, wlist, h, menuListHelp,
...
static void drawEditorBar(Editor *editor)
{
    double x, y, w, h;
    x = 0;
    y = winHeight - editor->menuHeight - editor->barHeight;
    w = winWidth;
    h = editor->barHeight;
    int newSelect = selectBar(GenUIID(0), x, y, w, h, editor->fileName, editor->fileCount,
                              editor->curSelect);
...
static void drawEditorForm(EditorForm *form)
{
    ...
    while (curLine != NULL) {
        ...
        if (curLineIndex >= form->startLine && curLinePosY + textFontHeight >= 0) {
            ...
            MovePen(form->x + indexLength - TextStringWidth(lineIndex) - 0.1,
                    curLinePosY + GetFontDescent());
            DrawTextString(lineIndex);
            // Traverse line (list of tokens)
            drawCodeLine(form, curLine->datptr, form->x + indexLength, curLinePosY, form->w,
                         textFontHeight);
