...
void drawEditor(Editor *editor)
{
    ...
    for (idx = 1; idx <= editor->fileCount; idx++) {
        ...
        if (scale < 1) {
            form->viewProgress = vertivalScrollBar(GenUIID(idx), form->x, form->y, form->w, form->h, scale,
                                                   form->viewProgress);
...
static void drawEditorComplete(Editor *editor)
{
    ...
    if (offset == 0 && form->realCaretPos.c >= 1) {
        ...
        if (selection != -1) {
            ...
            addTrace(form->urStack, ADD, form->realCaretPos.r, form->realCaretPos.c + 1, form->realCaretPos.r,
...
void drawEditorSelection(EditorForm *form)
{
    ...
    while (nowRol <= rRC.r) {
        double x = form->x + indexLength, y = form->h - textFontHeight * (nowRol - form->startLine + 1),
